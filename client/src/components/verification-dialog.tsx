...
export function VerificationDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const checkApiResults = async () => {
    ...
    try {
      ...
      console.log('Fazendo requisição para:', `${apiUrl}?codcoor=${userData.cod}`);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="dark:bg-gray-800 dark:text-white">
        <DialogHeader>
          ...
          <DialogDescription>
            Verificando a conexão com a API de clientes usando token de autenticação e parâmetro codcoor
          </DialogDescription>
        </DialogHeader>
        <div className="p-4">
          {loading && (
          ...
          {clientCount !== null && !loading && !error && (
            <div className="bg-green-100 dark:bg-green-900/30 border border-green-300 dark:border-green-800 rounded-md p-4">
              <div className="flex items-center">
                ...
                <div>
                  <p className="font-medium text-green-700 dark:text-green-400">Conexão com API bem-sucedida!</p>
                  <p className="text-sm text-green-600 dark:text-green-300 mt-1">
                    Quantidade de clientes encontrados: <span className="font-bold">{clientCount}</span>
                  </p>
                </div>
              </div>
            </div>
